...
/*! A thread allows multiple threads of execution within the application
   The thread is used by creating a subclass and overriding the run() method.
   The thread can then be started executing via the start() method, which will execute the
   run() method concurrently. The thread will continue runing until either the run method completes, 
   or until the application is terminated. There will be no guarentee to determine in which order 
   statements in different threads will be executed relative to eachother. For this reason threads should 
   probably utilise Locks or mutexes to ensure there are no data conflicts. */
class Thread {

	/*! The real thread. Only gets instanciated by start() */
	pthread_t pthreadHandle;	

	/*! Will be set to True while the thread is running */
	bool running;	

	/*! Allows some syncrhonization with the running boolean */
	Lock runningLock;		
	...
	public:
		...
		/*! Begin execution of the thread
 		  ...
 		  \throws ThreadCreateFailedException This happens if the thread cannot be started */
		void start();
